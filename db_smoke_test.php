...
function clearAllResults(mysqli $conn): void {
    // Stored procedures can leave additional result sets.
...
$requiredColumns = [
    ...
    'MISSION' => ['mission_id', 'res_id', 'driver_id', 'vehicle_id', 'rendezvous_location', 'appointment_datetime', 'expected_duration', 'actual_start_datetime', 'actual_end_datetime', 'odometer_start', 'odometer_end', 'mission_status'],
...
// Minimums (per your selected option A for VEHICLE_RATE)
$minRows = [
    'CLIENT' => 10,
    'DRIVER' => 10,
    'VEHICLE' => 10,
...
// 6) Procedure behavior smoke (basic)

// update_mission_details should update mission_id=1
...
$odoEnd = 12500;
$status = 'C';
$stmt->bind_param('issiis', $missionId, $start, $end, $odoStart, $odoEnd, $status);
if (!$stmt->execute()) {
    $err = $stmt->error;
    $stmt->close();
...
$verify = $conn->query('SELECT actual_start_datetime, actual_end_datetime, odometer_start, odometer_end, mission_status FROM MISSION WHERE mission_id = 1');
if (!$verify) {
...
// cancel_mission should delete a disposable mission
...
$insert = $conn->prepare(
    'INSERT INTO MISSION (mission_id, res_id, driver_id, vehicle_id, rendezvous_location, appointment_datetime, expected_duration, mission_status) VALUES (?, 1, 1, 1, ?, ?, 1, ?)' 
);
if (!$insert) {
